...
export function handleBindDirective(path, name, value, state) {
  if (state.computeds[value]) {
    path.replaceWith(t.jSXAttribute(t.jSXIdentifier(name), t.jSXExpressionContainer(t.identifier(value))))
    return
  }
  path.replaceWith(
    t.jSXAttribute(
...
export function handleForDirective(path, value, definedInFor, state) {
  ...
  const member = state.computeds[prop]
    ? t.identifier(prop)
    : t.memberExpression(t.memberExpression(t.thisExpression(), getIdentifier(state, value)), t.identifier(prop))
